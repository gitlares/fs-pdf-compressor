...
4. Test changes on every platform they affect: Apple Silicon running macOS 14
   or later, Windows 11 x64, and/or x86_64 Linux.
...
Use Windows 11 with an x64 Python 3.12 installation, including under x64
...
.windows-build-venv\Scripts\python windows_app.py
```

See [Windows builds](docs/WINDOWS.md) for the Ghostscript and Inno Setup
requirements.
...
.build-venv/bin/python -m py_compile \
  native_app.py build_macos.py linux_app.py build_linux.py windows_app.py build_windows.py \
...
For Windows changes, run the **Build Windows x64** workflow, install its setup
executable on Windows 11 x64, and complete the three-profile, Explorer action,
Drop Zone, update-link, and Recycle Bin checks in
[Windows release testing](docs/WINDOWS_TESTING.md).
