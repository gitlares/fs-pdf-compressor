...
class ProcessOutputTests(unittest.TestCase):
    def test_compression_discards_process_output(self):
        with tempfile.TemporaryDirectory() as directory:
            original = Path(directory) / "original.pdf"
            original.write_bytes(b"original payload")
            received = {}

            def fail_compression(command, **kwargs):
            ...
            self.assertIs(received["stderr"], subprocess.DEVNULL)
    ...
    def test_compress_pdf_replaces_only_after_a_smaller_result(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            def write_smaller(command, **_kwargs):
                ...
                Path(output.removeprefix("-sOutputFile=")).write_bytes(b"%PDF-1.7\n%%EOF")
            ...
            self.assertEqual(metrics, {"original_size": 16, "saved_size": 2})
            self.assertFalse(Path(f"{original}.temp.pdf").exists())
            move_to_trash.assert_called_once()

    def test_compress_pdf_replaces_when_trash_fails(self):
        with tempfile.TemporaryDirectory() as directory:
            original = Path(directory) / "original.pdf"
            original.write_bytes(b"original payload")
